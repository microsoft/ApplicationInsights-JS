...
import {
    IAppInsightsCore, IConfiguration, IPlugin, ITelemetryItem, ITelemetryPluginChain, dateNow, objKeys
...
export class Statsbeat {
    constructor() {
        ...
        // Custom dimensions
        let _cikey: string | IPromise<string>;
        ...
        dynamicProto(Statsbeat, this, (_self, _base) => {
            _self.initialize = (config: IConfiguration & IConfig, core: IAppInsightsCore, extensions: IPlugin[], pluginChain?:ITelemetryPluginChain, endpoint?: string) => {
                ...
                _sender = new Sender();
                let statsBeatConfig = {...config};
                statsBeatConfig.instrumentationKey = INSTRUMENTATION_KEY;
                _sender.initialize(statsBeatConfig, core, extensions, pluginChain);
                _statsbeatMetrics = {};
                _config = config;
                _isEnabled = true;
                if (_isEnabled) {
