...
export class SenderTests extends AITestClass {
    ...
    public registerTests() {
        ...
        this.testCase({
            name: 'Sender: flush with callback should call callback and return true',
            test: () => {
                // Arrange
                ...
                // Act
                const result = this._sender.flush(true, (flushComplete) => {
                    callbackCalled = true;
                    callbackResult = flushComplete;
                });

                // Assert
                QUnit.assert.equal(result, true, "flush should return true when callback is provided");
                
                // Wait for async callback
                this.clock.tick(10);
                
                QUnit.assert.equal(callbackCalled, true, "callback should be called");
                QUnit.assert.equal(callbackResult, true, "callback should be called with true for success");
            }
        });

        this.testCase({
            name: 'Sender: flush without callback should return promise when async=true',
            test: () => {
                // Act
                const result = this._sender.flush(true);

                // Assert
                QUnit.assert.ok(result, "flush should return a promise when no callback provided and async=true");
                QUnit.assert.ok(typeof result.then === 'function', "result should have a then method (be a promise)");
                ...
                QUnit.assert.equal(promiseResolved, true, "promise should be resolved");
                QUnit.assert.equal(promiseResult, true, "promise should resolve with true for success");
            }
        });

        this.testCase({
            name: 'Sender: flush when paused should call callback with false',
            test: () => {
                // Arrange
                this._sender.pause();
                let callbackCalled = false;
                ...
                // Assert
                QUnit.assert.equal(result, true, "flush should return true when callback is provided even when paused");
                
                // Wait for async callback
                this.clock.tick(10);
                
                QUnit.assert.equal(callbackCalled, true, "callback should be called");
                QUnit.assert.equal(callbackResult, false, "callback should be called with false when paused");
        ...
        this.testCase({
            name: 'Sender: flush when paused without callback should return resolved promise with false',
            test: () => {
                // Arrange
                this._sender.pause();
                ...
                // Assert
                QUnit.assert.ok(result, "flush should return a promise when no callback provided");
                QUnit.assert.ok(typeof result.then === 'function', "result should have a then method (be a promise)");
                ...
                QUnit.assert.equal(promiseResolved, true, "promise should be resolved");
                QUnit.assert.equal(promiseResult, false, "promise should resolve with false when paused");
