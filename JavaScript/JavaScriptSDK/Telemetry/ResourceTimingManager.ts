...
module Microsoft.ApplicationInsights.Telemetry {
    ...
    export class ResourceTimingManager {
        ...
        private Init() {
            if (this.enabled && this.IsPerformanceApiSupported() && Telemetry.PageViewPerformance.isPerformanceTimingSupported()) {
                this.SetInterval();
            }
        }
        ...
        public Report() {
            ...
            for (var i = 0; i < Math.min(resources.length, this.maxResourcesTrackedPerPage); i++) {
                ...
                var dependency = new Telemetry.RemoteDependencyData(Util.newId(), url, UrlHelper.getPathName(url), resource.duration, true, 200, null, RemoteDependencyData.ResourceDependancyName);
