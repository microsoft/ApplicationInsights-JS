...
import { 
    ...
    isReadableSpan 
} from "../../../../src/OpenTelemetry/trace/utils";
...
import { createDistributedTraceContext, createW3cTraceState, throwOTelNotImplementedError } from "../../../../types/applicationinsights-core-js";
...
function _createOtelApi(): IOTelApi {
    let otelCfg = {
        traceCfg: {},
        errorHandlers: {
    ...
    return createOTelApi(otelApiCtx, "default");
}
...
export class TraceUtilsTests extends AITestClass {
    ...
    private addCreateNonRecordingSpanTests(): void {
        ...
        this.testCase({
            name: "createNonRecordingSpan: non-recording span should not trigger onEnd",
            test: () => {
                // Arrange
                let onEndCalled = false;
                const testApi = _createOtelApi();
                const span = createNonRecordingSpan(testApi, "test", this._validSpanContext);

                // Act
                span.end();

                // Assert
                Assert.ok(!onEndCalled, "onEnd should not be called for non-recording span");
