import { IOTelHrTime, OTelTimeInput, toISOString } from "@microsoft/applicationinsights-core-js";
import {
    ICachedValue, WellKnownSymbols, createCachedValue, createDeferredCachedValue, getKnownSymbol, getPerformance, isArray, isDate,
    isNullOrUndefined, isNumber, mathFloor, mathRound, objDefine, objDefineProps, objFreeze, perfNow, strRepeat, strRight, throwTypeError
...
function _finalizeHrTime(hrTime: IOTelHrTime) {
    ...
    objDefine(hrTime, getKnownSymbol(WellKnownSymbols.toStringTag), { g: _toString });
...
function _createUnixNanoHrTime(unixNano: number): IOTelHrTime {
    const hrTime = [0, 0] as IOTelHrTime;
    objDefineProps(hrTime, {
        0: {
            l: createDeferredCachedValue(() => mathFloor(unixNano / NANOS_IN_MILLIS)),
            w: false
        },
        1: {
            l: createDeferredCachedValue(() => unixNano % NANOS_IN_MILLIS),
            w: false
        },
        unixNano: {
            v: unixNano,
...
function _createHrTime(seconds: number, nanoseconds: number): IOTelHrTime {
    ...
    objDefine(hrTime, "unixNano", {
        l: createDeferredCachedValue(() => (seconds * NANOS_IN_MILLIS) + nanoseconds),
        w: false,
        e: false
    });
