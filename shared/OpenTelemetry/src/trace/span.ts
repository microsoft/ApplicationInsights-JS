...
export function createSpan(spanCtx: IOTelSpanCtx, orgName: string, kind: OTelSpanKind): IReadableSpan {
    ...
    if (isRecording) {
        attributes = createDeferredCachedValue(() => createAttributeContainer(otelCfg, spanCtx.attributes));
    }
    ...
    objDefineProps(theSpan, {
        ...
        attributes: {
            g: () => {
                return attributes ? attributes.v.toAttributes() : objFreeze({});
            }
        },
        links: {
            g: () => {
