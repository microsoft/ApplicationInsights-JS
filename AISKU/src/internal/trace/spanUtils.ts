...
import {
    CtxTagKeys, IContextTagKeys, IDependencyTelemetry, IRequestTelemetry, RemoteDependencyDataType, RemoteDependencyEnvelopeType,
    RequestDataType, RequestEnvelopeType, createTelemetryItem, urlGetPathName
...
import {
    ...
    IReadableSpan, ITelemetryItem, MicrosoftClientIp, OTelAttributeValue, SEMATTRS_DB_NAME, SEMATTRS_DB_OPERATION, SEMATTRS_DB_STATEMENT,
    ...
    getUrl, getUserAgent, hrTimeToMilliseconds, isSqlDB, isSyntheticSource
} from "@microsoft/applicationinsights-core-js";
import {
    ILazyValue, arrIncludes, asString, getLazy, isNullOrUndefined, isString, objForEachKey, strLower, strStartsWith, strSubstring
...
function _populateTagsFromSpan(telemetryItem: ITelemetryItem, span: IReadableSpan, contextKeys: IContextTagKeys): void {
...
export function createTelemetryItemFromSpan(core: IAppInsightsCore, span: IReadableSpan): ITelemetryItem | null {
    ...
    if (telemetryItem) {
        ...
        _populateTagsFromSpan(telemetryItem, span, contextKeys);
        ...
        // Truncate properties
        if (baseData.id) {
            baseData.id = strSubstring(baseData.id, 0, eMaxPropertyLengths.NINE_BIT);
        }
        
        if (baseData.name) {
            baseData.name = strSubstring(baseData.name, 0, eMaxPropertyLengths.TEN_BIT);
        }
        
        if (baseData.responseCode) {
            baseData.responseCode = strSubstring(asString(baseData.responseCode), 0, eMaxPropertyLengths.TEN_BIT);
        }
        
        if (baseData.data) {
            baseData.data = strSubstring(asString(baseData.data), 0, eMaxPropertyLengths.THIRTEEN_BIT);
        }
        
        if (baseData.type) {
            baseData.type = strSubstring(asString(baseData.type), 0, eMaxPropertyLengths.TEN_BIT);
        }
        
        if (baseData.target) {
            baseData.target = strSubstring(asString(baseData.target), 0, eMaxPropertyLengths.TEN_BIT);
        }

        if (baseData.properties) {
            objForEachKey(baseData.properties, (prop, value) => {
                if (isString(value) && value.length > eMaxPropertyLengths.THIRTEEN_BIT) {
                    baseData.properties[prop] = strSubstring(value, 0, eMaxPropertyLengths.THIRTEEN_BIT);
                }
            });
        }
    }
