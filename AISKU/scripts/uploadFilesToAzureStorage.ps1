...
foreach($file in $files) {
    Set-AzureStorageBlobContent -Container $containerName -File (Join-Path $releaseFromDir -ChildPath $file) -Blob ($file) -Context $azureContext -Properties @{CacheControl = $cacheControlValue; ContentType = $contentType}
}
