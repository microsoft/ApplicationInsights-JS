...
export class SpanUtilsTests extends AITestClass {
    ...
    public registerTests() {
        ...
        this.addPropertyTruncationTests();
    ...
    private addPropertyTruncationTests(): void {
        this.testCase({
            name: "Property Truncation: baseData.id limited to 512 chars",
            test: () => {
                // Arrange
                this._trackCalls = [];
                // const longId = "x".repeat(600); // Would be used if we could set spanId manually
                
                // Act
                const span = this._ai.startSpan("truncation-test", {
                    kind: eOTelSpanKind.CLIENT
                });
                // Note: spanId is generated, so we test that it doesn't exceed limit
                span?.end();

                // Assert
                const item = this._trackCalls[0];
                Assert.ok((item.baseData as any).id.length <= 512, "ID should be truncated to 512 chars");
            }
        });

        this.testCase({
            name: "Property Truncation: baseData.name limited to 1024 chars",
            test: () => {
                // Arrange
                this._trackCalls = [];
                const longName = "operation-" + "x".repeat(1100);
                ...
                // Assert
                const item = this._trackCalls[0];
                Assert.ok((item.baseData as any).name.length <= 1024, "Name should be truncated to 1024 chars");
            }
        });

        this.testCase({
            name: "Property Truncation: baseData.data limited to 8192 chars",
            test: () => {
                // Arrange
                this._trackCalls = [];
                const longStatement = "SELECT " + "x".repeat(9000) + " FROM table";
                
                // Act
                const span = this._ai.startSpan("long-query", {
                    kind: eOTelSpanKind.CLIENT,
                    attributes: {
                        "db.system": "mysql",
                        "db.statement": longStatement
                    }
                });
                span?.end();
                ...
                // Assert
                const item = this._trackCalls[0];
                Assert.ok((item.baseData as any).data.length <= 8192, "Data should be truncated to 8192 chars");
            }
        });

        this.testCase({
            name: "Property Truncation: baseData.type and target limited to 1024 chars",
            test: () => {
                // Arrange
                this._trackCalls = [];
                const longHost = "subdomain." + "x".repeat(1100) + ".example.com";
                
                // Act
                const span = this._ai.startSpan("long-target", {
                    kind: eOTelSpanKind.CLIENT,
                    attributes: {
                        "http.method": "GET",
                        "http.url": `https://${longHost}/api`,
                        "net.peer.name": longHost
                    }
                });
                span?.end();
                ...
                // Assert
                const item = this._trackCalls[0];
                if ((item.baseData as any).target) {
                    Assert.ok((item.baseData as any).target.length <= 1024, "Target should be truncated to 1024 chars");
                }
                Assert.ok((item.baseData as any).type.length <= 1024, "Type should be truncated to 1024 chars");
            }
        });
    }
    ...
    private addEdgeCaseTests(): void {
        ...
        this.testCase({
            ...
            test: () => {
                ...
                Assert.ok(props["long.value"].length <= 8192, "Should be truncated to reasonable length");
            }
        });
