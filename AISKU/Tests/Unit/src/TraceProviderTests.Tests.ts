            ...
            test: () => {
                ...
                if (provider) {
                    Assert.ok(isAvailable, 
                        "Provider should be available after complete initialization");
                    
                    // If available, it should support core operations
                    if (isAvailable) {
                        Assert.ok(typeof provider.createSpan === 'function', 
                            "Available provider should have createSpan");
                        Assert.ok(typeof provider.activeSpan === 'function', 
                            "Available provider should have activeSpan");
                        Assert.ok(typeof provider.setActiveSpan === 'function', 
                            "Available provider should have setActiveSpan");
                    }
                }
            }
        });
        ...
        this.testCase({
            ...
            test: () => {
                ...
                if (provider) {
                    ...
                    // Test activeSpan
                    Assert.doesNotThrow(() => {
                        const active = provider.activeSpan();
