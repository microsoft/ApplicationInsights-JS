        ...
        this.testCase({
            ...
            test: () => {
                ...
                Assert.equal(childContext.traceId, parentContext.traceId,
                    "Child span should inherit parent's traceId");
            ...
            test: () => {
                ...
                return createPromise<void>((resolve) => {
                    setTimeout(() => {
                        ...
                        // Assert
                        if (capturedContext) {
                            Assert.equal(asyncContext.traceId, capturedContext.traceId,
                                "Async span should inherit captured traceId");
                        }
    ...
    private addContextPropagationTests(): void {
        this.testCase({
            ...
            test: () => {
                ...
                const provider = this._ai.core.getTraceProvider();
                if (provider) {
                    // Set trace1 as active
                    provider.setActiveSpan(trace1Root!);
                }
