...
import { Sender } from '../JavaScriptSDK/Sender';
...
import { _InternalLogging, LoggingSeverity, _InternalMessageId, _InternalLogMessage, Util, IConfig } from 'applicationinsights-common';
...
export class AppInsights implements IAppInsightsInternal, IAppInsights {
    ...
    constructor(config: IConfig) {
        ...
        this.context = new TelemetryContext(configGetters);
    ...
    public trackEvent(name: string, properties?: Object, measurements?: Object) {
        try {
            var eventTelemetry = new Event(name, properties, measurements);
            var data = new Data<Event>(Event.dataType, eventTelemetry);
            var envelope = new Envelope(data, Event.envelopeType);
            this.context.track(envelope);
        } catch (e) {
            _InternalLogging.throwInternal(LoggingSeverity.CRITICAL,
    ...
    public trackDependency(id: string, method: string, absoluteUrl: string, command: string, totalTime: number, success: boolean, resultCode: number, properties?: Object, measurements?: Object) {
        if (this.config.maxAjaxCallsPerView === -1 ||
            ...
            var envelope = new Envelope(dependencyData, RemoteDependencyData.envelopeType);
            this.context.track(envelope);
    ...
    */
    ...
    public trackException(exception: Error, handledAt?: string, properties?: Object, measurements?: Object, severityLevel?: SeverityLevel) {
        try {
            ...
            var exceptionTelemetry = new Exception(exception, properties, measurements, severityLevel);
            var data = new Data<Exception>(Exception.dataType, exceptionTelemetry);
            var envelope = new Envelope(data, Exception.envelopeType);
            this.context.track(envelope);
        } catch (e) {
            _InternalLogging.throwInternal(LoggingSeverity.CRITICAL,
    ...
    public trackMetric(name: string, average: number, sampleCount?: number, min?: number, max?: number, properties?: Object) {
        try {
            ...
            var envelope = new Envelope(data, Metric.envelopeType);
    ...
    */
    public trackTrace(message: string, properties?: Object, severityLevel?: SeverityLevel) {
        try {
            ...
            var envelope = new Envelope(data, Trace.envelopeType);
   ...
   */
    ...
    */
    private SendCORSException(properties: any) {
        var exceptionData = Exception.CreateSimpleException(
            "Script error.",
        ...
        var data = new Data<Exception>(Exception.dataType, exceptionData);
        var envelope = new Envelope(data, Exception.envelopeType);
        this.context.track(envelope);
    }
