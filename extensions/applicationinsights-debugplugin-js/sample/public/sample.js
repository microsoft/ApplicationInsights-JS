const myFunc = () => {
  ...
  const toTrack = ['trackEvent', 'trackTrace'];

  const debugPluginInstance = new Microsoft.ApplicationInsights.DebugPlugin.DebugPlugin();
  ...
  const testBtnFns = {
    trackEvent: () => { window.appInsights.trackEvent({name: 'some event'}); },
    trackPageView: () => { window.appInsights.trackPageView({name: 'some page'}); },
    trackPageViewPerformance: () => { window.appInsights.trackPageViewPerformance({name : 'some page', url: 'some url'}); },
    trackException: () => { window.appInsights.trackException({exception: new Error('some error')}); },
    trackTrace: () => { window.appInsights.trackTrace({message: 'some trace'}); },
    trackMetric: () => { window.appInsights.trackMetric({name: 'some metric', average: 42}); },
    trackDependencyData: () => { window.appInsights.trackDependencyData({absoluteUrl: 'some url', responseCode: 200, method: 'GET', id: 'some id'}); },
    triggerErrorImmediate: () => {
      throw new Error('testErrorImmediate');
    },
    triggerErrorAsync: () => {
      setTimeout(() => {throw new Error('testErrorAsync')}, 1000);
    },
    throwInternal: () => {
      window.appInsights.core.logger.throwInternal(1, 42069, 'test internal err');
    },
    send: () => {
      const idx = window.appInsights.core._extensions.findIndex(v => v.identifier === 'ChannelControllerPlugin');
      window.appInsights.core._extensions[idx].getChannelControls()[0][0].triggerSend();
    }
  }

  for (const [key, fn] of Object.entries(testBtnFns)) {
